  ...
  return (
    <SettingsRow className="flex-col items-stretch gap-2 px-3 py-3">
      <div className="flex items-center justify-between gap-2">
        <p className="text-[12px] font-semibold text-brand-ink">Sender authentication</p>
        <button
          type="button"
          onClick={() => void load(true)}
          disabled={loading}
          className="inline-flex items-center gap-1 text-[11px] font-semibold text-brand-ink-soft hover:text-brand-ink disabled:opacity-50"
        >
      ...
      {loading && !health ? (
        <p className="text-[11px] text-brand-ink-faint">Checking DNS…</p>
      ) : health && auth ? (
        <>
          <p className="text-[11px] text-brand-ink-soft">{auth.label}</p>
          ...
          {health.issues.length > 0 ? (
            <ul className="list-disc space-y-0.5 pl-4 text-[11px] text-brand-ink-soft">
              {health.issues.map((i) => (
                <li key={i.id}>{i.label}</li>
              ))}
            </ul>
          ) : (
            <p className="text-[11px] text-brand-green">No sender issues detected.</p>
          )}
        </>
      ) : (
        <p className="text-[11px] text-brand-ink-faint">Could not load sender health.</p>
      )}
    </SettingsRow>
  );
}
